...
const MESSAGE_TEMPLATES = {
  ...
  PREVENTIVE: {
    "-5": {
      email:
        "Напоминаем: через 5 дней наступает срок платежа по займу №{{creditNumber}}. Сумма к оплате: {{amount}} {{currency}}.",
    },
    "-3": {
      email:
        "Уважаемый клиент, напоминаем о приближающемся сроке платежа. Просим убедиться в наличии достаточных средств для его исполнения.",
    ...
    "-2": {
      email:
        "Через 2 дня наступает срок платежа. Избегайте негативных последствий - обеспечьте наличие средств на счете.",
    },
    "-1": {
      email:
        "ВНИМАНИЕ: Завтра последний день для внесения платежа по займу №{{creditNumber}}. Несвоевременная оплата повлечет финансовые и репутационные последствия.",
      email:
        "ВНИМАНИЕ: Завтра последний день для внесения платежа по займу №{{creditNumber}}. Несвоевременная оплата повлечет финансовые и репутационные последствия.",
    },
    0: {
  ...
  EARLY_DELAY: {
    ...
    2: {
      email:
        "Ваш займ №{{creditNumber}} переведен в статус 'Просрочен'. Доступ к новым займам ЗАБЛОКИРОВАН. Для разблокировки немедленно погасите задолженность.",
      email:
        "Ваш займ №{{creditNumber}} переведен в статус 'Просрочен'. Доступ к новым займам ЗАБЛОКИРОВАН. Для разблокировки немедленно погасите задолженность.",
    },
    3: {
    ...
    6: {
      email:
        "СРОЧНО: Сегодня последний день перед передачей вашего дела специализированному отделу взыскания. Погасите долг для предотвращения эскалации мер.",
    },
    7: {
      email:
        "УВЕДОМЛЕНИЕ О ПЕРЕВОДЕ ВАШЕГО ДЕЛА: Ваша задолженность передана в отдел взыскания. Начислены штрафные санкции согласно п.4.7 Договора. По вопросу урегулирования свяжитесь с нами в течение 24 часов.",
  ...
  MEDIUM_DELAY: {
    8: {
      email:
        "УВЕДОМЛЕНИЕ ОТ СЛУЖБЫ ВЗЫСКАНИЯ: Зафиксировано уклонение от погашения займа №{{creditNumber}}. Предлагаем последнюю возможность урегулирования до применения полного комплекса мер взыскания.",
      email:
        "УВЕДОМЛЕНИЕ ОТ СЛУЖБЫ ВЗЫСКАНИЯ: Зафиксировано уклонение от погашения займа №{{creditNumber}}. Предлагаем последнюю возможность урегулирования до применения полного комплекса мер взыскания.",
    },
    9: {
    ...
    11: {
      email:
        "ВНИМАНИЕ: Для вашего займа №{{creditNumber}} активирован протокол предаукционной оценки. После продажи долга условия взыскания определяются новым кредитором.",
    },
    12: {
      email:
        "ПОСЛЕДНЕЕ ПРЕДЛОЖЕНИЕ ПО УРЕГУЛИРОВАНИЮ: В связи с подготовкой к продаже вашего долга предлагаем возможность пролонгации/реструктуризации. Данное предложение действительно 24 часа. Дальнейшие условия будут определяться новым держателем долга.",
    ...
    13: {
      email:
        "КРИТИЧЕСКИЙ СТАТУС займа №{{creditNumber}}. Ваше бездействие зафиксировано и будет использовано при составлении юридического заключения.",
    },
    14: {
      email:
        "УВЕДОМЛЕНИЕ О ФИНАЛЬНОЙ ПОДГОТОВКЕ К АУКЦИОНУ: Ваш долг по займу №{{creditNumber}} включен в пакет активов для продажи профессиональным взыскателям. После передачи долга все вопросы урегулирования будут в компетенции нового кредитора.",
  ...
  LATE_DELAY: {
    ...
    17: {
      email:
        "ВНИМАНИЕ: Осталось {{remainingDays}} дней до аукциона по продаже вашего долга. После продажи все переговоры становятся невозможными.",
    },
    18: {
      email:
        "ВНИМАНИЕ: Осталось {{remainingDays}} дней до аукциона по продаже вашего долга. После продажи все переговоры становятся невозможными.",
    ...
    21: {
      email:
        "Ваш долг будет продан через {{remainingDays}} дней. После продажи все предложения по урегулированию аннулируются.",
      email:
        "Ваш долг будет продан через {{remainingDays}} дней. После продажи все предложения по урегулированию аннулируются.",
    },
    22: {
